package com.example.smartschedule.core.presentation.viewmodel
...
import com.example.smartschedule.core.presentation.user.UserUiState
            ...
            try {
                userRepository.getAllUsersWithResult().collect { userListRes ->
                    userListRes.fold(
                        ...
                        onError = { error ->
                            _uiState.value = UserUiState.Error(ErrorState.Companion.fromThrowable(error))
                        }
                    )
                }
            } catch (e: Exception) {
                try {
                    ...
                    _uiState.value = UserUiState.Error(ErrorState.Companion.fromThrowable(fallbackError))
                }
            }
        }
    }
